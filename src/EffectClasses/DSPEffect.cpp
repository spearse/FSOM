...
namespace fsom{
  ...
  return m_parameterList;
	...
	void DSPEffect::add_parameter(const std::string& IdName, float lowerBound, float upperBound, float value){
		...
		tempBPUnit->add_breakpoint(TVPair(static_cast<float>(m_dataStruct.attatchedRegion->get_duration()),value));
  ...
  return m_bypass;
	...
	void DSPEffect::set_parameters(ParameterList list){
		...
		for(ParameterList::iterator it = list.begin(); it != list.end();++it){
			...
			ParameterPtr old((*it).second);
			
			ParameterPtr p( new Parameter(old->get_duration(),old->get_name(),old->get_lowerBound(),old->get_upperBound(),old->get_value(),old->get_breakpoints()));
			
			
			m_parameterList.insert(std::pair<std::string, ParameterPtr>((*it).first,p ) ) ;
		}
	...
	void DSPEffect::print_parameters(){
		
		std::cout << "***"<< m_IdName << " parameters ***" <<std::endl;
		
		
		for(ParameterList::iterator it = m_parameterList.begin(); it != m_parameterList.end();++it){
