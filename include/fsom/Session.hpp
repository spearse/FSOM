...
class Session : public MetaDataStore{
  ...
  ///Currently active session name
	...
	Parameter create_parameter_from_node(TiXmlElement* element);
	DSPEffectPtr create_effect_from_node(TiXmlElement* element,RegionPtr region);
	GeneratorPtr create_generator_from_node(TiXmlElement* element);
	SynthesisModulePtr create_module_from_node(TiXmlElement* element);
public:
	Session();
	~Session();
private:
